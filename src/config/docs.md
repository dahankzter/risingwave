...
| unsafe_enable_local_fs_access | Whether to allow local filesystem connectors such as `posix_fs` and `fs`. Disabled by default because these connectors can access files on the frontend host. | false |
