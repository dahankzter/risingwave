    ...
    pub async fn update_user(
        ...
        update_fields.iter().for_each(|&field| match field {
            ...
            PbUpdateField::Inherit => {
                user.can_inherit = Set(update_user.can_inherit.unwrap_or(true))
            }
        });
        ...
        if revoke_user.is_super {
            ...
            for (obj, actions) in revoke_items {
                ...
                for action in actions {
                    let effective_grantor = if principals.contains(&owner) {
                        owner
                    } else {
                        let mut grant_option_principal = None;
                        for principal in &principals {
                            let exists = UserPrivilege::find()
                                ...
                                .is_some();
                            if exists {
                                grant_option_principal = Some(*principal);
                                break;
                            }
                        }
                        grant_option_principal.ok_or_else(|| {
                            MetaError::permission_denied(format!(
                                "user {} does not have privilege {:?} with grant option on object {}",
                    ...
                    root_user_privileges.extend(
                        UserPrivilege::find()
                            .select_only()
                            .columns([user_privilege::Column::Id, user_privilege::Column::UserId])
                            .filter(user_privilege::Column::GrantedBy.eq(effective_grantor))
