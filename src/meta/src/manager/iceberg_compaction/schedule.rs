...
impl CompactionTrack {
    ...
    fn record_commit(&mut self, committed_snapshot: Option<IcebergCommittedSnapshot>) {
        if let Some(committed_snapshot) = committed_snapshot {
            self.latest_observed_snapshot = Some(committed_snapshot);
        }
    ...
    fn mark_dispatched(&mut self, task_id: u64, now: Instant) {
        let (task_type, pending_commit_count_at_dispatch, gc_watermark_snapshot) = match &self.state
        {
            CompactionTrackState::PendingDispatch {
                task_type,
...
#[derive(Debug, Clone)]
enum SinkUpdateKind {
    /// A normal sink commit. It records the latest observed snapshot and
    /// increases the pending snapshot count.
    Commit {
        committed_snapshot: Option<IcebergCommittedSnapshot>,
    },
...
impl SinkUpdateKind {
    fn apply_to_track(self, track: &mut CompactionTrack, now: Instant) {
        match self {
            SinkUpdateKind::Commit { committed_snapshot } => {
                track.record_commit(committed_snapshot);
            }
...
impl IcebergCompactionManager {
    ...
    pub(super) async fn start_manual_compaction(
        ...
        {
            return Err(anyhow!(
                "manual iceberg compaction for sink {} is blocked by snapshot expiration",
                sink_id
            )
            .into());
        }
    ...
    pub fn clear_iceberg_maintenance_by_sink_id(&self, sink_id: SinkId) {
        let waiter = {
            ...
            guard.sink_schedules.remove(&sink_id);
            guard.snapshot_expiration_sink_ids.remove(&sink_id);
            guard.finish_snapshot_expiration(sink_id);
